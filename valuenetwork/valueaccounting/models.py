...
class Unit(models.Model):
    ...
    def default_unit(cls):
        try:
            items = Unit.objects.get(name='Each')
        except:
            items = Unit(
                unit_type='quantity',
                abbrev='',
...
class EconomicResourceType(models.Model):
    ...
    unit = models.ForeignKey(Unit, default=Unit.default_unit,
    ...
    def save(self, *args, **kwargs):
        #unique_slugify(self, self.name)
        no_none_unit = self.unit or self.unit_of_use or Unit.default_unit()
        if not self.unit:
            self.unit = no_none_unit
        if not self.unit_of_use:
            self.unit_of_use = no_none_unit
...
class EconomicResource(models.Model):
    ...
    def transform_form(self, data=None):
        ...
        unit = self.resource_type.unit or Unit.default_unit()
        qty_help = ""
        if unit:
    ...
    def use_event_form(self, data=None):
        from valuenetwork.valueaccounting.forms import InputEventForm
        prefix=self.form_prefix()
        unit = self.resource_type.directional_unit("use") or Unit.default_unit()
        qty_help = " ".join(["unit:", unit.abbrev, ", up to 2 decimal places"])
    ...
    def cite_event_form(self, data=None):
        from valuenetwork.valueaccounting.forms import InputEventForm
        prefix=self.form_prefix()
        unit = self.resource_type.directional_unit("cite") or Unit.default_unit()
        qty_help = " ".join(["unit:", unit.abbrev, ", up to 2 decimal places"])
        return InputEventForm(qty_help=qty_help, prefix=prefix, data=data)
...
class ProcessTypeResourceType(models.Model):
    ...
    def create_commitment_for_process(self, process, user, inheritance):
        ...
        unit = self.resource_type.directional_unit(self.event_type.relationship) or Unit.default_unit()
        #import pdb; pdb.set_trace()
...
class Exchange(models.Model):
    ...
    def slots_with_detail(self):
        ...
        more_slots = transfers.exclude(transfer_type__in=slots)
        for more in more_slots:
            if more.transfer_type not in slots:
                slots.append(more.transfer_type)
        ...
        for slot in slots:
            #slot.xfers = []
            slot.total = 0
            # TAG maybe we want to sort/cluster by agent instead?
            slot.xfers = list(transfers.filter(transfer_type=slot).order_by("-transfer_date"))
            for transfer in slot.xfers:#transfers:
    ...
    def __unicode__(self):
        ...
        if event:
            ...
            if event.unit_of_quantity:
                unit = (event.unit_of_quantity or Unit.default_unit()).name
        else:
...
class Commitment(models.Model):
    ...
    def input_event_form(self, data=None):
        from valuenetwork.valueaccounting.forms import InputEventForm
        prefix=self.form_prefix()
        qty_help = " ".join(["unit:", (self.unit_of_quantity or Unit.default_unit()).abbrev, ", up to 2 decimal places"])
        return InputEventForm(qty_help=qty_help, prefix=prefix, data=data)
...
class EconomicEvent(models.Model):
    ...
    def distribution_quantity_formatted(self):
        unit = self.own_or_resource_type_unit() or Unit.default_unit()
